...
handler.join = function (msg, session, next) {
    ...
    self.app.rpc.game.RS_erShiYiDianRemote.join(session, msg, self.app.get('serverId'), rid, true, function (users) {
        if (users.length == 3) {
...
var onUserLeave = function (app, session) {
    ...
    app.rpc.game.RS_erShiYiDianRemote.kick(session, session.uid, app.get('serverId'), session.get('rid'), function (data) {
        console.log(data);
    });
};