...
handler.join = function (msg, session, next) {
    ...
    self.app.rpc.game.R_gaoSuSaiCheRemote.join(session, msg, self.app.get('serverId'), rid, true, function (users) {
        if (users.length == 3) {
...
var onUserLeave = function (app, session) {
    ...
    app.rpc.game.R_gaoSuSaiCheRemote.kick(session, session.uid, app.get('serverId'), session.get('rid'), function (data) {
        console.log(data);
    });
};