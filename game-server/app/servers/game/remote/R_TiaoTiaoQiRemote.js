module.exports = function (app) {
    return new TiaoTiaoQiRemote(app);
};

var consts = require('../../../consts/consts');

var TiaoTiaoQiRemote = function (app) {
    this.app = app;
...
TiaoTiaoQiRemote.prototype.join = function (msg, sid, name, flag, cb) {
    ...
    if (users.length == 2) {
        ...
        param = { msgId: consts.ttq.MsgId.msgId_playersInfo };
        param.content = JSON.stringify(this.profileManager.getProfileMessage(users[1]));
        push(this.channelService, "onmessage", param, uid0, sid0);
        // 发给另外一个用户users[1] －》 发送801消息。
        param.content = JSON.stringify(this.profileManager.getProfileMessage(users[0]));
...
TiaoTiaoQiRemote.prototype.kick = function (uid, sid, rid, cb) {
    ...
    if (!!channel) {
        ...
        var winmsg = {
            msgId:consts.ttq.MsgId.msgId_gameOver,
